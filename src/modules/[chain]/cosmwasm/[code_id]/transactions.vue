...
<template>
    <div>
        ...
        <div class="text-center mb-4">
            ...
            <label @click="showFunds()" for="modal-contract-funds" class="btn btn-mw-primary btn-xs text-xs mr-2">{{
                $t('cosmwasm.btn_funds') }}</label>
            <label class="btn btn-mw-primary btn-xs text-xs mr-2" for="modal-contract-states" @click="showState()">
                {{ $t('cosmwasm.btn_states') }}
            </label>
            <label for="modal-contract-query" class="btn btn-mw-primary btn-xs text-xs mr-2" @click="showQuery()">
                {{ $t('cosmwasm.btn_query') }}
            </label>
            <label for="wasm_execute_contract" class="btn btn-mw-primary btn-xs text-xs mr-2"
                @click="dialog.open('wasm_execute_contract', { contract: contractAddress })">
                {{ $t('cosmwasm.btn_execute') }}
            </label>

            <label for="wasm_migrate_contract" class="btn btn-mw-primary btn-xs text-xs mr-2"
                @click="dialog.open('wasm_migrate_contract', { contract: contractAddress })">
                {{ $t('cosmwasm.btn_migrate') }}
            </label>

            <label for="wasm_update_admin" class="btn btn-mw-primary btn-xs text-xs mr-2"
                @click="dialog.open('wasm_update_admin', { contract: contractAddress })">
                {{ $t('cosmwasm.btn_update_admin') }}
            </label>

            <label for="wasm_clear_admin" class="btn btn-mw-primary btn-xs text-xs mr-2"
